...
bool valid(const char side[])
{
  long l = strlen (side);
  long i;
  for (i = 0; i < l; i++)
  {
      if (IsDecimal(side[i]) == false) return false;
      if ((strlen(side) > 1) && side[0] - '0' == 0) return false;
  }
  return true;
}
...
long convert(const char side[])
{
  long n = atoi (side);
  if ((valid(side)) == true) return n;
  else return -1;
}
...
long triangle(const char sa[], const char sb[], const char sc[])
{
  ...
  if ((a + b == c) || (a + c == b) || (b + c == a)) return Flat;
  if (((a == b) && (c < a + b)) || ((a == c) && (b < c + a)) || ((c == b) && (a < c + b))) return Isosceles;
  if ((((a*a) + (b*b)) == (c*c)) || (((a*a) == ((b*b) + (c*c)))) || (((a*a) + (c*c)) == (b*b))) return Right;
  if ((a > b + c) || (b > a + c) || (c > a + b)) return Impossible;
  else return Scalene;
}
